...
def determine_directcast_config_severity_and_description(attr_value: str) -> tuple:
    ...
    if 'DirectCast' in attr_value and 'Config' in attr_value:
        if 'SecureString' in attr_value:
            return 'MEDIUM', 'Password retrieved from DirectCast Config SecureString - Review configuration source security'
        elif 'String' in attr_value:
            return 'MEDIUM', 'Password retrieved from DirectCast Config String - Review configuration source security'
        else:
            return 'MEDIUM', 'Password retrieved from DirectCast Config - Review configuration source security'
    else:
        return 'INFO', 'Password retrieved from DirectCast configuration source - Review if source is secure'
